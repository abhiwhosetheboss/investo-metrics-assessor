...
export const saveAnalysis = async (data: { userId: string; analysisData: AnalysisResult }): Promise<{ success: boolean; message: string; id?: string }> => {
  try {
...
export const getSavedAnalyses = async (): Promise<AnalysisResult[]> => {
  try {
    // Check if user is authenticated
    const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
    if (sessionError || !sessionData.session) {
      throw new Error("Authentication required to retrieve saved analyses");
    }
    ...
    // Transform database records to AnalysisResult format
    return data.map(record => ({
      ...record.data,
      id: record.id,
      startupName: record.startup_name,
      investibilityScore: record.investibility_score,
      overallRisk: record.overall_risk,
      createdAt: record.created_at
    }));
  } catch (error) {
    console.error("Error in getSavedAnalyses:", error);
    return [];
  }
};
...
export const deleteSavedAnalysis = async (id: string): Promise<{ success: boolean; message: string }> => {
  try {
    ...
    const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
    if (sessionError || !sessionData.session) {
      throw new Error("Authentication required to delete saved analysis");
    }
    ...
    console.error("Error in deleteSavedAnalysis:", error);
    return {
      success: false,
